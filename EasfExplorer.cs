 ...
 *  - Xóa block
 *  - Lưu lại file .big (rebuild + re-encrypt + SHA-256)
    ...
    class MainForm : Form
    {
        ...
        void LoadFile(string path)
        {
            try
            {
                ...
                if (raw.Length < OUTER_HEADER_SIZE + EASF_HEADER_SIZE)
                    throw new Exception("File quá nhỏ hoặc không phải định dạng EASF.");

                // Kiểm tra outer SHA-256
                byte[] embeddedHash = new byte[32];
                Buffer.BlockCopy(raw, 0, embeddedHash, 0, 32);
                byte[] computedHash = ComputeSHA256(raw, OUTER_HEADER_SIZE, raw.Length - OUTER_HEADER_SIZE);
                bool outerOk = BytesEqual(embeddedHash, computedHash);

                // Parse blocks
                var parsed = new List<EasfBlock>();
                int offset = OUTER_HEADER_SIZE;
                int idx    = 0;
                ...
                while (offset + EASF_HEADER_SIZE <= raw.Length)
                {
                    ...
                    if (plain != null && plain.Length > 0)
                    {
                        ...
                        byte[] calcDigest = ComputeSHA256(plain, 0, digestLen);
                ...
                dirty           = false;
                RefreshTree();
                Text = string.Format("EASF Explorer – {0}{1}", Path.GetFileName(path),
                                     outerOk ? "" : " [SHA-256 ngoài KHÔNG khớp!]");
                SetStatus(string.Format("Đã tải {0} block(s). Outer SHA-256: {1}",
                                        blocks.Count, outerOk ? "OK ✓" : "KHÔNG KHỚP ⚠"));
            }
            catch (Exception ex)
            {
        ...
        void SaveFile(string path)
        {
            ...
            try
            {
                using (MemoryStream body = new MemoryStream())
                {
                    ...
                    byte[] outerHeader = new byte[OUTER_HEADER_SIZE];
                ...
                RefreshTree();
                SetStatus(string.Format("Đã lưu: {0}  ({1} block(s))", path, blocks.Count));
            }
            catch (Exception ex)
            {
